 ...
 * 7. Newlines: <br>
 */
export function mrkdwnToHtml(mrkdwn) {
    if (!mrkdwn || typeof mrkdwn !== 'string')
        return '';
    ...
    html = html.replace(/(^|[\s])\*([^*]+)\*($|[\s])/g, '$1<b>$2</b>$3');
...
function parseContextBlock(block) {
    const result = { html: '<br><small>', plain: '' };
 ...
 */
export function transformSlackToMatrix(payload) {
    let html = '';
    let plain = '';
 ...
 */
export function isValidBase64Url(encoded) {
 ...
 */
export function decodeMatrixUrl(encodedPath) {
